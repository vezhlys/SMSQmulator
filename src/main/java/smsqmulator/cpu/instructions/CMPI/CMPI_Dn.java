 ...
 */
public class CMPI_Dn implements InstructionSet
{
    @Override
            ...
            if(sz == 0)
            {
                ...
                i = new smsqmulator.cpu.Instruction()    // byte sized compare
                {
                    ...
                    public final void execute(int opcode,smsqmulator.cpu.MC68000Cpu cpu)
                    {
                        ...
                        if ((s & 0x80) != 0)
                            s|=0xffffff00;
                        int d=cpu.data_regs[opcode & 0x07]&0xff;// des value
                        if ((d & 0x80) != 0)
                            d|=0xffffff00;
                        int r = d-s;
            ...
            else if(sz == 1)
            {
                ...
                i = new smsqmulator.cpu.Instruction()
                {
                    ...
                    { 
                        ...
                        int d=cpu.data_regs[opcode & 0x07]&0xffff;// des value
                        if ((d & 0x8000) != 0)
                            d|=0xffff0000;
                        int r = d-s;
                        cpu.reg_sr&=0xfff0;            // X bit not affected by cmp
            ...
            else
            {
                ...
                i = new smsqmulator.cpu.Instruction()
                {
                    ...
                    public final void execute(int opcode,smsqmulator.cpu.MC68000Cpu cpu)
                    {
                        ...
                        cpu.pc_reg+=2;
                        int d=cpu.data_regs[opcode & 0x07];// des value
                        int r = d-s;
                        cpu.reg_sr&=0xfff0;            // X bit not affected by cmp
                        boolean Sm = (s< 0);                // source is neg
