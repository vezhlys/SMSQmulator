...
/**
 * MOVE where source is immediate DATA and destination is .W absolute word.
 * 
  00ssxxxyyymmmrrr
 * s = size bit
 * x=dest register 001
...
public class MOVEAnT8 implements InstructionSet
{
    @Override
            ...
            else if(sz == 1)
            {
                ...
                i = new smsqmulator.cpu.Instruction()
                {
                    ...
                    public final void execute(int opcode,smsqmulator.cpu.MC68000Cpu cpu)
                    {
                        ...
                        int displacement = cpu.readMemoryLongPC();
                        cpu.pc_reg+=2;
                        cpu.writeMemoryWord(displacement, s);
                        cpu.reg_sr&=0xfff0;
                        if (s==0)
                        {
                            cpu.reg_sr+=4;
