...
/**
 * MOVE where source is d8(Pc,Xn) and destination is d16(AN).
 * 
  00ssxxxyyymmmrrr
 * s = size bit
 * x=dest register 
...
public class MOVEAnX5 implements InstructionSet
{
    @Override
            ...
            else if(sz == 1)
            {
                ...
                i = new smsqmulator.cpu.Instruction()
                {
                    ...
                    public final void execute(int opcode,smsqmulator.cpu.MC68000Cpu cpu)
                    {
                        ...
                        cpu.writeMemoryShort(cpu.addr_regs[(opcode>>9)&7]+ cpu.readMemoryWordPCSignedInc(), s);
                        cpu.reg_sr&=0xfff0;
                        if (s==0)
                        {
                            cpu.reg_sr+=4;
