...
/**
 * The LSR instruction where data to be shifted is in memory.
 * 1110001011mmmrrr = 0xe2C0  
 * 
 * mmm = mode 010
...
public class LSRmem implements InstructionSet
{
    @Override
    public final void register(final smsqmulator.cpu.MC68000Cpu cpu2)
    {
        ...
        i = new smsqmulator.cpu.Instruction()   
        {
            ...
            { 
                ...
                int lastOut=val&1;                       
                val>>>=1;                                       // shift one bit doown - value can't be negative
                cpu.writeMemoryWord(address,val);          // write back
                cpu.reg_sr&=0xffe0;                        // all status flags set to 0 (value can't be negative)
                if (val==0)
            ...
            public DisassembledInstruction disassemble(int address, int opcode,smsqmulator.cpu.MC68000Cpu cpu)
            {
                ...
                return new DisassembledInstruction(address, opcode, "lsr" + Size.Word.ext(), src);
            }   
