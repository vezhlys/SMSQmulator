...
/**
 * MOVE where source is d16(AN) and destination is DN.
 * 
 * 00ssxxxyyymmmrrr
 * s = size bit
 * x=dest register
 * y = destination mode =000
 * m =source mode 101
 * r= source register
 * so 00ssxxx000mmm000 = base , s can 01 byte, 11 word,10 long
 * so base = 0x1000 for byte,0x2000 for long,0x3000 for word, 
 * 
 * @author and copyright for my code (c) Wolfgang Lenerz 2013 - 2016.
 * 
...
public class MOVEAnD1 implements InstructionSet
{
    @Override
        ...
        for(int sz = 0; sz < 3; sz++)
        {
            if(sz == 0)
            {
                ...
                i = new smsqmulator.cpu.Instruction()
                {
                    ...
                    public final void execute(int opcode,smsqmulator.cpu.MC68000Cpu cpu)
                    {
                        ...
                        int dest=(opcode>>9)&7;
                        int d=cpu.data_regs[dest]&0xffffff00;
                        cpu.data_regs[dest]=d|s;
                        cpu.reg_sr&=0xfff0;
                        if (s==0)
                        {
                            cpu.reg_sr+=4;
            ...
            {                                                   // move word
                ...
                i = new smsqmulator.cpu.Instruction()
                {
                    ...
                    public final void execute(int opcode,smsqmulator.cpu.MC68000Cpu cpu)
                    {
                        int s=cpu.readMemoryWord(cpu.addr_regs[opcode &7] + cpu.readMemoryWordPCSignedInc());
                        int dest=(opcode>>9)&7;
                   //     int d=cpu.data_regs[dest]&0xffff0000;
                     //   cpu.data_regs[dest]=d|s;
                        cpu.data_regs[dest]&=0xffff0000;
                        cpu.data_regs[dest]|=s;
                        cpu.reg_sr&=0xfff0;
                        if (s==0)
                        {
                            cpu.reg_sr+=4;
