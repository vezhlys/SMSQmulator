...
/**
 * MOVE where source is d16(Pc) and destination is d8(AN,Xn).
 * 
  00ssxxxyyymmmrrr
 * s = size bit
 * x=dest register 
...
public class MOVEAnY6 implements InstructionSet
{
    @Override
        ...
        for(int sz = 0; sz < 3; sz++)
        {
            if(sz == 0)
            {
                ...
                i = new smsqmulator.cpu.Instruction()
                {
                    ...
                    {
                        int s=(cpu.readMemoryByte(cpu.pc_reg*2+ cpu.readMemoryWordPCSignedInc()));// the first readsigned is faster than a read word and we have to cut off anyway
