...
public class MOVEA4 implements InstructionSet
{
    @Override
        ...
        for(int sz = 1; sz < 3; sz++)
        {
            ...
            else
            {
                ...
                i = new smsqmulator.cpu.Instruction()
                {
                    ...
                    {
                        int reg=opcode &7;
                        cpu.addr_regs[reg]+=4;             // LEAVE THIS HERE - DO NOT INCREASE REG AFETER THE OPERATION : MOVE.L(A7)+,A7 would be wrong result
                        cpu.addr_regs[(opcode>>9)&7]=cpu.readMemoryLong(cpu.addr_regs[reg]-4);
                    }
                   @Override
                    public DisassembledInstruction disassemble(int address, int opcode,smsqmulator.cpu.MC68000Cpu cpu)
                    {
