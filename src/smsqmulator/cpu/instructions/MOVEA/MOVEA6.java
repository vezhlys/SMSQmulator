...
/**
 * MOVE where source is d16(AN) and destination is AN.
 * 
 * 
 * 00ssxxx001mmmrrr
 * s = size bit
...
public class MOVEA6 implements InstructionSet
{
    @Override
        ...
        for(int sz = 1; sz < 3; sz++)
        {
            if(sz == 1)
            {
                ...
                {   
                    ...
                    {
                        cpu.addr_regs[(opcode>>9)&7]=cpu.readMemoryWordSigned(cpu.addr_regs[opcode &7] + cpu.readMemoryWordPCSignedInc());
                    }
                    @Override
