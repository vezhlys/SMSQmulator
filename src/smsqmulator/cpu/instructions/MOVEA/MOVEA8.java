...
/**
 * MOVE where source is absolute .W and destination is An.
 * 
 * 00ssxxx001mmmrrr
 * s = size bit
 * x=dest register 
...
public class MOVEA8 implements InstructionSet
{
    @Override
        ...
        for(int sz = 1; sz < 3; sz++)
        {
            if(sz == 1)
            {
                ...
                i = new smsqmulator.cpu.Instruction()
                {
                    ...
                    {
                        int s = cpu.readMemoryWordPCSignedInc();         // read the address, sign extend
                        cpu.addr_regs[(opcode>>9)&7]=cpu.readMemoryWordSigned(s);// get content, sign exten
                    }
                    @Override
            ...
            else
            {
                ...
                i = new smsqmulator.cpu.Instruction()
                {
                    ...
                    public final void execute(int opcode,smsqmulator.cpu.MC68000Cpu cpu)
                    {
                                int s = cpu.readMemoryWordPCSignedInc();   // read the address
                                cpu.addr_regs[(opcode>>9)&7]=cpu.readMemoryLong(s);
                    }
                   @Override
                    public DisassembledInstruction disassemble(int address, int opcode,smsqmulator.cpu.MC68000Cpu cpu)
                    {
