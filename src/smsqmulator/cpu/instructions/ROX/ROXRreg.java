...
/**
 * The ROXR instruction where data to be rotated is in a data reg and the rotate count is in another data reg.
 * 1110ccc0ss110rrr = 0xe030   
 * 
 * mmm = mode 010
...
public class ROXRreg implements InstructionSet
{
    @Override
            ...
            else if (sz==1)                                     // word sized shift
            {
                ...
                i = new smsqmulator.cpu.Instruction()   
                {
                    ...
                    public final void execute(int opcode,smsqmulator.cpu.MC68000Cpu cpu)
                    {
                        ...
                        for (int cnt=0;cnt<nbrShifts;cnt++)
                        {
                            lastOut=val&0x1;                    // lowestest bit before shift
                            val>>>=1;                           // shift one bit down
                            if (xBit!=0)
                                val|=0x8000;
                            xBit=lastOut;
            ...
            else
            {
                ...
                i = new smsqmulator.cpu.Instruction()   
                {
                    ...
                    { 
                        ...
                        cpu.data_regs[opcode&0x7]=val;     //  fill it in with new value
                        cpu.reg_sr&=0xfff0;                // all status flags set to 0, except for X
                        if (val<0)                // ms bit set
                            cpu.reg_sr+=8;                 // so set N flag if (val==0)    
                        else if (val==0)
